...
The `[DONE]` sentinel is unique to OpenAI ChatCompletions. The wire adapter emits two chunks for the done event: the final data frame + the `[DONE]` frame.
...
5. For SDK round-trip: import `sdkAdapter` from `../sdk-adapter.js`, add `sdkRequest` and `expectedSdkResult` (partial match)
